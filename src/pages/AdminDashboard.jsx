...
import { Activity, Loader2, RefreshCw, Trash2, Plus, Calendar, CheckSquare } from "lucide-react";
import axios from "axios";
import toast from 'react-hot-toast'
const AdminDashboard = () => {
  // ==========================================
  // ১. গ্লোবাল ও লোকাল স্টেটস
  // ==========================================
  ...
  // এপিআই থেকে আসা রিয়েল ডেটা স্টেটস (ডিফল্ট খালি অ্যারে রাখা হয়েছে ক্র্যাশ এড়াতে)
  const [labs, setLabs] = useState([]);
  const [students, setStudents] = useState([]);
  const [teacher, setTeacher] = useState({ name: "", email: "", joinToken: "" });

  // নতুন ল্যাব তৈরি করার ফর্ম স্টেট
  const [newLab, setNewLab] = useState({ title: "", date: new Date().toISOString().split('T')[0], totalProblems: 4 });
  const [newLabScores, setNewLabScores] = useState({}); // { studentId: score }

  const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/v1';
  const token = localStorage.getItem("token");

  // Axios কনফিগারেশন হেডার
  const config = { headers: { Authorization: `Bearer ${token}` } };

  // ল্যাব কাউন্ট ট্র্যাক করে ডিফল্ট টাইটেল সেট করা (সেফলি চেক করা হচ্ছে)
  useEffect(() => {
  ...
  // ==========================================
  // ২. ডেটা লোড করার মেথড (Fetch Initial Data)
  // ==========================================
  const fetchDashboardData = async () => {
    try {
      setIsLoading(true);
      
      // ১. টিচারের নিজস্ব প্রোফাইল ও ল্যাব লিস্ট আনা
      const profileRes = await axios.get(`${baseUrl}/auth/me`, config);
      if (profileRes.data && profileRes.data.success) {
        // ব্যাকএন্ড যদি ডিরেক্ট 'result' বা 'data' পাঠায় সেটার সেফটি হ্যান্ডলিং
        const tData = profileRes.data.data || profileRes.data.result;
        if (tData) {
          setTeacher({
            name: tData.name || "",
            email: tData.email || "",
            joinToken: tData.joinToken || tData.sectionId?.joinToken || ""
          });
          setLabs(tData.labs || []);
        }
      }
  ...
  // ==========================================
  // ৩. কোর ফাংশন ও এপিআই হ্যান্ডলারস (ফিক্সড রাউটস)
  // ==========================================
  ...
  // 🛠️ ফিক্সড রাউট: তোমার ব্যাকএন্ড রাউটার মিল রেখে পরিবর্তন করা হলো
const handleUpdateScores = async (studentId) => {
  try {
    const res = await axios.put(`${baseUrl}/admin/students/${studentId}/marks`, { 
      updatedScores: editScores // আপনার ব্যাকএন্ড কন্ট্রোলারের রিকোয়েস্ট বডির সাথে মিল রেখে 'updatedScores' ব্যবহার করা হয়েছে
    }, config);

    if (res.data.success) {
      setStudents((prev) => 
        prev.map((s) => (s._id === studentId ? { ...s, scores: editScores } : s))
      );
      setExpandedStudentId(null);
    ...
    toast.error("Failed to update marks. Please check the connection or data format.");
  }
};
...
const addTodayLab = async () => {
  ...
  try {
    ...
    // যদি ব্যাকএন্ড এরর পাঠায়, তবে সেটি দেখানোর ব্যবস্থা
    toast.error(error.response?.data?.message || "An error occurred.");
  }
};

  // 🛠️ ফিক্সড রাউট: router.delete('/admin/users/:id') অনুযায়ী পরিবর্তন
const handleRemoveStudent = async (studentId) => {
  const confirmed = window.confirm("Are you sure you want to remove this student from the section?");
...
const handleRegenerateToken = async () => {
  try {
    ...
    const newToken = res.data?.joinToken;
  ...
  // 🛠️ ফিক্সড রাউট: router.post('/admin/danger/clear') অনুযায়ী পরিবর্তন
const handleClearSection = async () => {
  // নিশ্চিত হওয়ার জন্য টোস্ট কনফার্মেশন বা ব্রাউজার কনফার্ম ব্যবহার করা যেতে পারে
  const confirmed = window.confirm(
    " Danger! Are you sure you want to permanently remove all students and their scores? This action cannot be undone."
  );
  ...
  if (confirmed) {
    try {
      ...
      toast.error("Failed to clear section. Please try again.");
    }
  }
};
  ...
  const calculateTotal = (studentScores) => {
    ...
    labs.forEach(lab => {
      if (lab && lab.id) {
        totalSolved += (safeScores[lab.id] || 0);
      }
    });
              ...
              <div className="bg-white p-6 rounded-2xl border border-teal-100/40 shadow-xl space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="text" value={newLab.title} onChange={e => setNewLab({...newLab, title: e.target.value})} className="w-full bg-[#EDFBF8] border border-teal-100/50 px-4 py-2.5 rounded-xl text-xs font-semibold focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-gray-400 mb-1 uppercase tracking-wider">Date</label>
                    <input type="date" value={newLab.date} onChange={e => setNewLab({...newLab, date: e.target.value})} className="w-full bg-[#EDFBF8] border border-teal-100/50 px-4 py-2.5 rounded-xl text-xs font-semibold focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-gray-400 mb-1 uppercase tracking-wider">Total Problems</label>
                    <input type="number" value={newLab.totalProblems} onChange={e => setNewLab({...newLab, totalProblems: parseInt(e.target.value) || 0})} className="w-full bg-[#EDFBF8] border border-teal-100/50 px-4 py-2.5 rounded-xl text-xs font-semibold focus:outline-none" />
                  </div>
                </div>

                {/* DYNAMIC STUDENTS FORM LIST ROWS */}
              ...
              <table className="w-full text-left border-collapse min-w-[700px]">
                <thead>
                  <tr className="bg-[#EDFBF8] border-b border-teal-100/30 text-[11px] text-gray-400 font-bold uppercase tracking-wider select-none">
                    ...
                    {labs?.map((lab) => (
                      <th key={lab.id || lab._id} className="py-3 px-4 text-center">
                    ...
                    students.map((student) => {
                      ...
                      return (
                        <React.Fragment key={student._id}>
                          <tr onClick={() => handleStudentClick(student)} className={`hover:bg-cyan-50/20 cursor-pointer transition-colors ${isExpanded ? "bg-cyan-50/30" : ""}`}>
                              ...
                              <td colSpan={(labs?.length || 0) + 2} className="bg-gray-50/60 p-5 border-y border-teal-50/50">
                                <div className="flex flex-col gap-4">
                                  ...
                                  <div className="flex flex-wrap gap-3">
                                    {labs?.map((lab) => (
                                      <div key={lab.id || lab._id} className="flex flex-col items-center bg-white p-2 rounded-xl border border-gray-100 shadow-sm w-20">
                                        <span className="text-[10px] font-bold text-gray-400 mb-1 truncate w-full text-center">{lab.title}</span>
                                        <input type="number" value={editScores[lab.id] ?? 0} onChange={(e) => handleScoreChange(lab.id, e.target.value, lab.totalProblems)} className="w-full text-center font-bold text-sm bg-[#EDFBF8] border border-teal-100 rounded-lg py-1 focus:outline-none" />
                                      </div>
                                    ))}
                                  </div>
                                </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-sm font-semibold text-teal-950">
                {/* <tr className="bg-cyan-50/10">
                  <td className="py-4 px-6 font-bold text-gray-800">{teacher.name || "Teacher"} (You)</td>
                  <td className="py-4 px-6 font-medium text-gray-400">{teacher.email}</td>
                  <td className="py-4 px-6">
                    <span className="bg-cyan-100 text-cyan-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Teacher</span>
                  </td>
                  <td className="py-4 px-6 text-right text-xs text-gray-400 font-normal italic select-none">
                    N/A
                  </td>
                </tr> */}
                {students?.map((student) => (
                  <tr key={student._id} className="hover:bg-teal-50/10 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{student.email.split('@')[0]}</span>
                    </td>
                    <td className="py-4 px-6 text-right">
                      <button onClick={() => handleRemoveStudent(student._id)} className="text-red-500 hover:text-red-700 p-2 rounded-xl hover:bg-red-50 transition-all flex items-center gap-1 ml-auto font-bold text-xs">
      ...
      <RefreshCw className="w-4 h-4" /> Regenerate Token
            ...
            <div className="bg-white p-6 rounded-2xl border border-red-100 shadow-sm space-y-4">
              ...
              <div className="flex items-center gap-3">
                <button onClick={handleClearSection} className="px-5 py-3 bg-red-600 hover:bg-red-700 text-white font-bold text-sm rounded-xl transition-all shadow-md">
